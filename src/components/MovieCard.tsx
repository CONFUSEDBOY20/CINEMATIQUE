...
export const MovieCard = memo(({ movie }: { movie: any }) => {
  ...
  return (
    <div
      className="group relative cursor-pointer w-44 md:w-56 shrink-0 snap-center will-change-transform h-full"
      onClick={handleClick}
    >
      <div className="relative aspect-[2/3] w-full rounded-2xl overflow-hidden border border-white/5 bg-brand-gray shadow-lg">
        <LazyImage
          src={movie.posterUrl}
          alt={movie.title}
          className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-[transform,opacity] duration-700"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-black via-[rgba(0,0,0,0.4)] to-transparent pointer-events-none" />
        ...
        {/* Hover Overlay — GPU accelerated via opacity/transform only */}
        <div className="absolute inset-0 bg-gradient-to-t from-brand-black via-brand-black/90 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4 z-10">
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              className={`p-1.5 rounded-full backdrop-blur border transition-colors ${isWatchlisted ? 'bg-brand-crimson/80 border-red-500/50 text-white' : 'bg-white/10 border-white/20 text-white hover:bg-white/20'}`}
            >
          ...
          <button className="flex items-center justify-center gap-2 bg-brand-crimson hover:bg-red-700 text-white py-2 rounded-full text-xs font-bold tracking-widest uppercase transition-colors shadow-lg border border-red-500/50">
